...
from criteo_api_api_experimental.api.gateway_api import GatewayApi
from criteo_api_api_experimental.api_client_builder import ApiClientBuilder
...
class TestGatewayApi:
  ...
  def before_each(self):
    self.client_id = os.environ.get("API_TEST_CLIENT_ID")
    self.client_secret = os.environ.get("API_TEST_CLIENT_SECRET")
    self.application_id = int(os.environ.get("API_TEST_APPLICATION_ID"))

    self.client = ApiClientBuilder.WithClientCredentials(clientId=self.client_id, clientSecret=self.client_secret)

  def test_environment_variables(self):
    assert len(self.client_id) > 0, "Environment variable \"API_TEST_CLIENT_ID\" not found."
    assert len(self.client_secret) > 0, "Environment variable \"API_TEST_CLIENT_SECRET\" not found."
    assert self.application_id > 0, "Environment variable \"API_TEST_APPLICATION_ID\" not found."
