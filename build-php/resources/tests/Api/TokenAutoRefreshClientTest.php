...
class TokenAutoRefreshClientTest extends TestCase
{
    ...
    public function testDoesNotInterceptRequestWhenNoAuthorizationHeader(): void
    {
        ...
        $response = (new Criteo\Marketing\Api\OAuthApi($mockClient))
            ->createToken($this->clientId, $this->clientSecret, $this->grantType);


        $this->assertInstanceOf("\Criteo\Marketing\Model\JwtModel", $response);
        $this->assertEquals($token, $response->getAccessToken());
    }

    public function testAlwaysSendValidBearerToken(): void
    {
    ...
    public function testDoesNotRefreshValidToken(): void
    {
        $targetPath = 'v1/valid/endpoint';
    ...
    public function testThrowsExceptionIfCannotRefreshToken(): void
    {
